...
fun EventView(onNavigateToHomeScreen: () -> Unit, eventViewModel: EventViewModel, navController: NavHostController = rememberNavController()) {
    ...
    Scaffold(
        ...
        floatingActionButton = {
            if (navController.currentBackStackEntryAsState().value?.destination?.route == "transactions") {
                FloatingActionButton(onClick = {
                    // TODO: add transaction adding logic
                }) {
        ...
        EventNavigationGraph(navController = navController, modifier = Modifier.padding(innerPadding), uiState = uiState)
    }
}

@Composable
private fun EventNavigationGraph(navController: NavHostController, modifier: Modifier, uiState: EventViewUiState) {
    NavHost(navController = navController, startDestination = "overview") {
        composable("overview") {
            OverviewScreen(modifier = modifier)
        }
        composable("debts") {
            DebtsScreen(modifier = modifier)
        }
